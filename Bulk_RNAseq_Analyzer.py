    ...
    if method == "log1p":
        ...
        if gene_lengths is not None:
            ...
            if len(common_genes) == 0:
                st.error("⛔ Gene IDs in counts data and gene length data do not match. TPM calculation was stopped.")
                st.stop()
            ...
            if _drop_rate >= 0.5:
                st.warning(
                    f"⚠️ TPM計算: 遺伝子長データと一致した遺伝子は {_n_common:,} / {_n_total_genes:,} 件です。"
                    f"{_n_dropped:,} 件（{_drop_rate:.0%}）が除外されました。"
                    "遺伝子IDの形式（例: Ensembl ID vs Gene Symbol）が一致しているか確認してください。"
                    if st.session_state.get("lang_display", "日本語") == "日本語" else
                    f"⚠️ TPM: Only {_n_common:,} / {_n_total_genes:,} genes matched the gene length data. "
                    f"{_n_dropped:,} genes ({_drop_rate:.0%}) were excluded. "
                    "Please check that gene ID formats match (e.g. Ensembl ID vs Gene Symbol)."
                )
            elif _n_dropped > 0:
                st.info(
                    f"ℹ️ TPM計算: {_n_common:,} / {_n_total_genes:,} 遺伝子を使用（{_n_dropped:,} 件は長さデータなし）。"
                    if st.session_state.get("lang_display", "日本語") == "日本語" else
                    f"ℹ️ TPM: Using {_n_common:,} / {_n_total_genes:,} genes ({_n_dropped:,} excluded — no length data)."
                )
            ...
            st.error("⛔ Gene length data is required for TPM calculation. Please provide gene length data in the Upload tab.")
            st.stop()
    elif method == "VST":
        try:
            ...
            import streamlit as st
            st.warning(
                f"⚠️ VST変換に失敗しました（{_vst_err}）。log1p(CPM)にフォールバックします。"
                "VSTを使用するにはPyDESeq2が正しくインストールされているか確認してください。"
                if st.session_state.get("lang_display", "日本語") == "日本語" else
                f"⚠️ VST failed ({_vst_err}). Falling back to log1p(CPM). "
                "Please ensure PyDESeq2 is correctly installed to use VST."
            )
            lib_size = counts_df.sum(axis=0)
            cpm = counts_df.div(lib_size, axis=1) * 1e6
            return np.log1p(cpm)
...
def run_deg(counts_df, metadata, ref_condition, test_condition, n_cpus=1):
    ...
    # Filter to the samples present in metadata (ref + test groups),
    # then fit the DESeq2 model and extract the specified contrast.
...
if st.session_state.get("upload_mode") is not None:
    ...
    if st.session_state.get("is_sample_data", False):
        _is_jp = st.session_state.get("lang_display", "日本語") == "日本語"
        msg = (
            "🧪 **サンプルデータを使用中 (Mouse, 12 samples, 3 groups)**\n\n"
            "このデータは機能デモ専用です。遺伝子発現パターンは人工的に設定されており、"
            "実際の実験データを代表するものではありません。"
        ) if _is_jp else (
            "🧪 **Using Sample Data (Mouse, 12 samples, 3 groups) — Demo only**\n\n"
            "This dataset is for demonstration purposes only. "
            "Expression patterns are artificially constructed and do not represent "
            "real experimental data or biological dispersion."
        )
        st.warning(msg)
...
""", unsafe_allow_html=True)
    ...
    def show_validation_card(df, is_jp):
        with st.container(border=True):
            ...
            if _has_negative:
                ...
                c1.markdown(f"- 🚨 **{'負値' if is_jp else 'Negative values'}**: " + (
                    f"{_n_neg:,}件の負値が含まれています。DESeq2はクラッシュします。データを確認してください。"
                    if is_jp else
                    f"{_n_neg:,} negative values detected. DESeq2 will crash. Please check your data."
                ))
            else:
                c1.markdown(f"- ✅ **{'負値なし' if is_jp else 'No negative values'}**: {'正常' if is_jp else 'OK'}")

            # 0b. NA/NaN check
            _has_na = df.isnull().any().any()
            if _has_na:
                _n_na = df.isnull().sum().sum()
                c1.markdown(f"- 🚨 **{'欠損値' if is_jp else 'Missing values (NA)'}**: " + (
                    f"{_n_na:,}件のNAが含まれています。解析前に除去または0埋めしてください。"
                    if is_jp else
                    f"{_n_na:,} NA values detected. Please remove or fill with 0 before analysis."
                ))
            else:
                c1.markdown(f"- ✅ **{'欠損値なし' if is_jp else 'No missing values'}**: {'正常' if is_jp else 'OK'}")
    ...
    if _mode == "single":
        ...
        if _is_jp:
            _norm_desc = {
                "log1p": "log1p(CPM) — 可視化・探索的解析向けの簡便法。DEG解析には raw counts を用い、相関解析やPCAではデータによってVSTがより適切な場合があります。",
                "CPM": "Counts Per Million — ライブラリサイズ補正",
                "TPM": "Transcripts Per Million — 遺伝子長補正あり。可視化・発現量の比較に使用。DEG解析にはraw countsを使用。サンプル間の組成比較には注意が必要。",
                "VST": "PyDESeq2を用いたDESeq2ライクな分散安定化変換（設定によりDESeq2と完全一致しない場合があります）",
            }
        else:
            _norm_desc = {
                "log1p": "log1p(CPM) — A simple transformation suitable for visualization and exploratory analysis; raw counts are used for DEG, and VST may be more appropriate for correlation analysis or PCA depending on the dataset.",
                "CPM": "Counts Per Million — library size correction",
                "TPM": "Transcripts Per Million — Gene-length corrected. For visualization only. DEG analysis uses raw counts. Caution: sensitive to compositional differences across samples.",
                "VST": "Variance Stabilizing Transformation — DESeq2-like implementation using PyDESeq2 (may not be identical to original DESeq2 depending on settings)",
            }
        ...
        if _norm_sel == "TPM":
            st.divider()
            st.markdown("#### 📏 " + ("遺伝子長データのアップロード" if _is_jp else "Upload Gene Lengths"))
            st.info("TPM計算には各遺伝子の長さ情報が必要です。1列目にGene ID、2列目に長さ（bp等）を持つファイル形式を想定しています。" if _is_jp else "TPM calculation requires gene lengths. Please upload a file with Gene IDs in the 1st column and lengths (bp, etc.) in the 2nd column.")
            uploaded_lengths = st.file_uploader("Gene lengths (CSV/TSV)", type=["csv", "tsv", "txt"], key="lengths_uploader")
            if uploaded_lengths:
                ...
                if len(len_df.columns) >= 1:
                    st.session_state["gene_lengths"] = len_df.iloc[:, 0].astype(float)
                    st.success(f"✅ {len(st.session_state['gene_lengths'])} genes length data loaded.")
                else:
                    st.error("Invalid gene length file format.")
            elif st.session_state.get("gene_lengths") is None:
                st.warning("⚠️ TPM requires gene length data. Please upload a gene length file above.")

        ul, ur = st.columns([1, 1], gap="large")
        with ul:
            btn_text = "🧪 サンプルデータで試す" if _is_jp else "🧪 Try with Sample Data"
            if st.button(btn_text, width="stretch"):
                with st.status("🎩 読み込み中...") as status:
        ...
        with ur:
            ...
            if st.session_state["counts_df"] is not None:
                ...
                st.session_state["metadata"] = pd.DataFrame.from_dict(
                ...
                st.divider()
                with st.expander(
                    "➕ 追加変数の設定（交互作用解析用・任意）" if _is_jp
                    else "➕ Additional variables (optional, for interaction analysis)",
                    expanded=False
                ):
                    if _is_jp:
                        st.markdown("""
...
""")
                    _n_extra_vars = st.number_input(
                        "追加変数の数" if _is_jp else "Number of additional variables",
                    ...
                    if _n_extra_vars > 0:
                        ...
                        for _vi in range(int(_n_extra_vars)):
                            _vname = st.text_input(
                                f"変数名 {_vi+1}" if _is_jp else f"Variable name {_vi+1}",
                                f"var{_vi+1}",
                        ...
                        if any(_extra_var_names):
                            st.markdown(
                                "**各サンプルの値を入力してください：**" if _is_jp
                                else "**Enter values for each sample:**"
                            )
                            _extra_cols_data = {}
                            ...
                            st.success(
                                f"✅ {len(_extra_var_names)}個の追加変数が設定されました。"
                                if _is_jp else
                                f"✅ {len(_extra_var_names)} additional variable(s) configured."
                            )
                ...
                if st.session_state["metadata"] is not None:
                    st.success("✅ " + ("データ準備完了！上の『DEG』タブに移動して解析を実行してください。" if _is_jp else "Data ready! Please go to the 'DEG' tab to run analysis."))
                ...
                with qct2:
                    ...
                    st.caption(
                        "ℹ️ このQCタブの相関ヒートマップとPCAは、常に **log1p(CPM)** で計算されます。"
                        "サイドバーの正規化設定（VST等）はDEG解析とVisualizationタブに適用されます。"
                        if _is_jp else
                        "ℹ️ Correlation heatmap and PCA in this QC tab are always computed using **log1p(CPM)**, "
                        "regardless of the normalization setting in the sidebar. "
                        "The sidebar normalization (e.g. VST) applies to DEG analysis and the Visualization tab."
                    )
    ...
    # ════════════════════════════════════════════════════════════════════
        ...
        if st.button(_ms_btn_text, width="stretch", key="multi_sample_btn"):
            with st.status("🎩 読み込み中...", expanded=True) as _msts:
        ...
        if _multi_files:
            ...
            if len(_multi_files) > 1:
                ...
                with _bc1:
                    st.info("💡 " + ("最初のStudyの設定（種、IDモード）を他のすべてのStudyにコピーできます。" if _is_jp else "Apply settings (Species, ID mode) from the first study to all others."))
                with _bc2:
                    if st.button("✨ " + ("全Studyに適用" if _is_jp else "Apply to All"), key="multi_batch_apply", use_container_width=True):
            ...
            if st.button("📥 全Studyを読み込む / Load All Studies",
                ...
                with st.status("🎩 読み込み中...", expanded=True) as _sts:
    ...
    if st.session_state["counts_df"] is None:
        ...
        with dl:
            with st.expander("⚙️ Plot Settings", expanded=True):
                lfc_t = st.slider(t("logfc_threshold", lang), 0.0, 5.0, lfc_t, 0.25, key="deg_lfc_in")
                padj_t = st.slider(t("pval_threshold", lang), 0.001, 0.1, padj_t, 0.005, key="deg_padj_in")
                st.session_state["lfc_t"] = lfc_t
                st.session_state["padj_t"] = padj_t
                st.session_state["deg_t"] = (lfc_t, padj_t)
                up_color = st.color_picker("Up color", st.session_state.get("up_color", "#E64B35"), key="deg_upc_in")
                down_color = st.color_picker("Down color", st.session_state.get("down_color", "#4DBBD5"), key="deg_dnc_in")
...
- DEG解析では、データセット内の全サンプルを用いて分散を推定し、その後に選択した比較群の結果を抽出しています。
""")
                else:
                    st.markdown("""
...
- DEG analysis estimates dispersion using all available samples in the dataset, and then extracts the selected contrast.
""")
# ── 解析制御ロジック ──────────────────────────────────────────
            ...
            if st.button("Analyze", type="primary", disabled=_analyze_disabled):
                ...
                if counts_per_group.min() < 2:
                    ...
                    try:
                        with st.status("🎩 DEG解析中...", expanded=True) as status:
                        ...
                        st.balloons()
                        st.success("✅ " + ("解析完了！『Visualization』または『Network』タブで結果を確認してください。" if _is_jp else "Analysis complete! Check results in the 'Visualization' or 'Network' tab."))
                        st.rerun()
        ...
        if len(_all_pairs) >= 2:
            ...
            if st.button("⚡ Run All Selected" if not _is_jp else "⚡ 選択したペアを一括実行", type="primary", key="batch_run_btn"):
                if not _selected_pairs:
                    ...
                    with st.status(f"🎩 {len(_selected_pairs)}ペアの一括解析中...", expanded=True) as _batch_status:
        ...
        if st.session_state.get("conditions") and len(st.session_state["conditions"]) >= 3:
            ...
            _venn_dir_opts = (
                ["Up（上昇）のみ", "Down（低下）のみ", "両方（Up + Down）"]
                if _is_jp else
                ["Up only", "Down only", "Both (Up + Down)"]
            )
            _venn_dir = st.radio(
                "変動方向フィルター" if _is_jp else "Direction filter",
                _venn_dir_opts,
                index=2,
                horizontal=True,
                key="venn_dir_filter",
                help=(
                    "「両方」を選ぶと、UPとDOWNが逆の遺伝子も共通DEGとして扱われます。"
                    "KEGGやGO解析に使う場合はUP・DOWNを分けることを推奨します。"
                    if _is_jp else
                    "Selecting 'Both' includes genes that are UP in one group and DOWN in another as shared DEGs. "
                    "For KEGG/GO analysis, it is recommended to separate UP and DOWN genes."
                )
            )
            ...
            if st.session_state.get("upload_mode") == "multi":
                if st.session_state.get("batch_deg_results"):
                    if _is_jp:
                        ...
                        st.info("💡 Multi Study mode: Venn will use the already-computed batch DEG results.")
                else:
                    if _is_jp:
                        st.warning("⚠️ 先に上の「Run All Selected」で各StudyのDEGを実行してください。")
                    else:
                        st.warning("⚠️ Please run 'Run All Selected' above before drawing the Venn diagram.")
                    ...
                    if _upload_mode_venn == "multi" and _batch_res:
                        ...
                        for _g in v_sel:
                            ...
                            if _matched_key:
                                ...
                                if _venn_dir in ["Up only", "Up（上昇）のみ"]:
                                    _venn_genes = set(_res_vs[
                                        (_res_vs["padj"] < padj_t) &
                                        (_res_vs["log2FoldChange"] > lfc_t)
                                    ].index)
                                elif _venn_dir in ["Down only", "Down（低下）のみ"]:
                                    _venn_genes = set(_res_vs[
                                        (_res_vs["padj"] < padj_t) &
                                        (_res_vs["log2FoldChange"] < -lfc_t)
                                    ].index)
                        ...
                        if _missing:
                            _warn_msg = (
                                f"以下のグループのDEG結果が見つかりません: {', '.join(_missing)}。"
                                "先に「Run All Selected」で一括DEGを実行してください。"
                                if _is_jp else
                                f"DEG results not found for: {', '.join(_missing)}. "
                                "Please run 'Run All Selected' first."
                            )
                ...
                if _n_sel == 0:
                    ...
                    with _venn_enr_col1:
                        if st.button("🧬 KEGG", type="primary", key="venn_kegg_btn", width="stretch"):
                            try:
                                import gseapy as gp
                                with st.status("🎩 KEGG解析中..."):
                                    _enr_venn_k = gp.enrichr(
                                        gene_list=_sel_genes,
                                        gene_sets=st.session_state["sp"]["gene_sets_kegg"],
                                        background=(st.session_state.get("qc_filtered_df") or st.session_state["counts_df"]).index.tolist(),
                                        outdir=None
                                    )
    ...
    st.divider()
    with st.expander(
        "🔬 交互作用解析（Advanced）" if _is_jp else "🔬 Interaction Analysis (Advanced)",
        expanded=False
    ):
        if _is_jp:
            st.markdown("""
...
**必要条件:**
- メタデータに交互作用変数（**因子型のみ対応**。数値を入力した場合は自動でカテゴリとして扱います）の列が必要
- 各群に4サンプル以上を推奨
...
**Requirements:**
- Metadata must contain the interaction variable (**categorical only**; numeric values are automatically treated as categories)
...
""")
        ...
        _counts_ia = st.session_state.get("qc_filtered_df", st.session_state.get("counts_df"))

        if _meta_ia is None or _counts_ia is None:
            st.info("💡 " + ("まずUploadタブでデータをアップロードしてください。" if _is_jp else "Please upload data in the Upload tab first."))
        else:
            _extra_cols = [c for c in _meta_ia.columns if c != "condition"]
            if not _extra_cols:
                st.warning("⚠️ " + (
                    "メタデータに'condition'以外の列が必要です。交互作用変数（age等）を追加してください。"
                    if _is_jp else
                    "Metadata must have columns other than 'condition'. Please add an interaction variable (e.g., age)."
                ))
            else:
                _ia_col1, _ia_col2 = st.columns(2)
                with _ia_col1:
                    _ia_var = st.selectbox(
                        "交互作用変数" if _is_jp else "Interaction variable",
                        _extra_cols,
                        key="ia_var_sel"
                    )
                    # 変数タイプの自動判定（数値かどうか）
                    ...
                    _ia_var_type_opts = (
                        ["連続変数（numeric）", "カテゴリ変数（factor）"]
                        if _is_jp else
                        ["Continuous (numeric)", "Categorical (factor)"]
                    )
                    _ia_var_type = st.radio(
                        "変数タイプ" if _is_jp else "Variable type",
                        _ia_var_type_opts,
                    ...
                    if not _ia_use_numeric:
                        ...
                        _ia_ref_level = st.selectbox(
                            "参照水準（Reference level）" if _is_jp else "Reference level of interaction variable",
                            _ia_var_levels,
                            index=0,
                            key="ia_ref_level_sel",
                            help=(
                                "この水準を基準として他の水準との交互作用効果を計算します。"
                                "生物学的に「対照」となる群（例: young）を選んでください。"
                                if _is_jp else
                                "This level is used as the baseline. "
                                "Other levels are compared against it. "
                                "Choose the biologically meaningful baseline (e.g., 'young')."
                            )
                        )
                        ...
                        st.caption(
                            f"📌 生成される係数: " +
                            ", ".join([f"`{_ia_var}[T.{l}]:condition[T.{{test}}]`" for l in _ia_other_levels])
                            if _is_jp else
                            f"📌 Coefficients to be generated: " +
                            ", ".join([f"`{_ia_var}[T.{l}]:condition[T.{{test}}]`" for l in _ia_other_levels])
                        )
                    else:
                        _ia_ref_level = None
                ...
                with _ia_col2:
                    ...
                    _ia_test = st.selectbox(
                        "Test condition",
                        [c for c in st.session_state.get("conditions", []) if c != _ia_ref],
                        key="ia_test_sel"
                    )
                ...
                if _ia_use_numeric:
                    st.info(
                        "📐 **連続変数モード**: 年齢などを数値のまま扱い、"
                        "「処置効果が変数の増加とともに線形に変化するか」を検定します。"
                        "単調増加・減少トレンドの検出に適しています。"
                        if _is_jp else
                        "📐 **Continuous mode**: The variable is treated as numeric. "
                        "Tests whether the treatment effect changes linearly with the variable (e.g., age). "
                        "Suitable for detecting monotonic trends."
                    )
                else:
                    st.info(
                        "🏷️ **カテゴリ変数モード**: 変数を独立した群として扱い、"
                        "「各群で処置効果が異なるか」を検定します。"
                        "生物学的に意味のある区切りがある場合に適しています。"
                        if _is_jp else
                        "🏷️ **Categorical mode**: The variable is treated as discrete groups. "
                        "Tests whether the treatment effect differs between groups. "
                        "Suitable when group boundaries are biologically meaningful."
                    )

                _ia_n_min = _meta_ia["condition"].isin([_ia_ref, _ia_test]).sum()
                # 交互作用変数の水準数を取得
                _ia_var_n_levels = len(_meta_ia[_ia_var].dropna().unique()) if _ia_var in _meta_ia.columns else 1
                # 必要最低サンプル数: 水準数 × 条件数(2) × 最低2サンプル
                _ia_n_required = _ia_var_n_levels * 2 * 2
                _ia_n_recommended = _ia_var_n_levels * 2 * 3
                if _ia_n_min < _ia_n_required:
                    st.error(f"⛔ " + (
                        f"サンプル数が不足しています（現在: {_ia_n_min}件）。"
                        f"交互作用変数の水準数{_ia_var_n_levels}に対し、最低{_ia_n_required}サンプル（各セルに2以上）が必要です。"
                        if _is_jp else
                        f"Insufficient samples ({_ia_n_min} total). "
                        f"With {_ia_var_n_levels} levels in the interaction variable, "
                        f"at least {_ia_n_required} samples are required (≥2 per cell)."
                    ))
                elif _ia_n_min < _ia_n_recommended:
                    st.warning(f"⚠️ " + (
                        f"サンプル数が少ない可能性があります（現在: {_ia_n_min}件）。"
                        f"各セル（条件 × 水準）に3サンプル以上（計{_ia_n_recommended}件以上）を推奨します。"
                        if _is_jp else
                        f"Sample size may be limited ({_ia_n_min} total). "
                        f"Recommended ≥3 samples per cell (group × level), i.e. ≥{_ia_n_recommended} total."
                    ))

                _ia_padj = st.number_input(
                    "padj閾値" if _is_jp else "padj threshold",
                    0.0, 1.0, 0.05, 0.005, key="ia_padj_t"
                )
                _ia_lfc = st.number_input(
                    "LFC閾値" if _is_jp else "LFC threshold",
                    0.0, 5.0, 0.5, 0.1, key="ia_lfc_t"
                )

                if st.button(
                    "▶ 交互作用解析を実行" if _is_jp else "▶ Run Interaction Analysis",
                    type="primary", key="ia_run_btn"
                ):
                    try:
                        from pydeseq2.dds import DeseqDataSet
                        from pydeseq2.ds import DeseqStats
                        ...
                        if _ia_use_numeric:
                            ...
                            try:
                                ...
                                st.error(
                                    f"⛔ 「{_ia_var}」を数値に変換できませんでした。"
                                    "カテゴリ変数モードに切り替えるか、値を数値にしてください。"
                                    if _is_jp else
                                    f"⛔ Could not convert '{_ia_var}' to numeric. "
                                    "Switch to Categorical mode or ensure all values are numbers."
                                )
                                st.stop()
                        else:
                        ...
                        with st.status(
                            "🎩 交互作用解析中..." if _is_jp else "🎩 Running interaction analysis...",
                            expanded=True
                        ):
                            ...
                            if not _all_coef_names:
                                st.error(
                                    "⛔ デザイン行列の係数名を取得できませんでした。"
                                    if _is_jp else
                                    "⛔ Could not retrieve design matrix coefficient names."
                                )
                                st.stop()
                            ...
                            if not _ia_coef_names:
                                st.error(
                                    "⛔ 交互作用項の係数が見つかりませんでした。"
                                    f"利用可能な係数: {_all_coef_names}"
                                    if _is_jp else
                                    "⛔ No interaction term coefficients found. "
                                    f"Available coefficients: {_all_coef_names}"
                                )
                                st.stop()
                            ...
                            for _coef in _ia_coef_names:
                                try:
                                    ...
                                    st.write(f"  ✅ 係数 / Coefficient: {_coef}")
                            ...
                            if not _ia_results_dict:
                                st.error(
                                    "⛔ 全ての交互作用項の検定に失敗しました。"
                                    if _is_jp else
                                    "⛔ All interaction term tests failed."
                                )
                                st.stop()
                        ...
                        st.error(f"Interaction analysis error: {_ia_e}")
                ...
                if st.session_state.get("ia_results") is not None:
                    ...
                    if len(_ia_coef_list) > 1:
                        _selected_term = st.selectbox(
                            "表示する交互作用項を選択" if _is_jp else "Select interaction term to display",
                            _ia_coef_list,
                            key="ia_term_sel"
                        )
                        st.caption(
                            f"📌 現在表示中: `{_selected_term}`  \n"
                            "各項は異なる年齢水準での交互作用効果を表します。"
                            if _is_jp else
                            f"📌 Showing: `{_selected_term}`  \n"
                            "Each term represents the interaction effect at a different level of the variable."
                        )
                    ...
                    _r1.metric("Significant genes", len(_sig_ia))
                    _r2.metric("Up", _ia_up)
                    _r3.metric("Down", _ia_dn)
                    ...
                    st.download_button(
                        "📥 " + ("交互作用解析結果 CSV" if _is_jp else "Download interaction results CSV"),
                        _sig_ia.to_csv(),
                    ...
                    # ── 可視化セクション ─────────────────────────────────
                    st.divider()
                    st.markdown("### 📊 " + ("可視化" if _is_jp else "Visualization"))

                    _viz_tabs = st.tabs([
                        "🔥 " + ("ヒートマップ" if _is_jp else "Heatmap"),
                        "📈 " + ("Interaction Line Plot" if _is_jp else "Interaction Line Plot"),
                        "🎯 " + ("LFC Scatter" if _is_jp else "LFC Scatter"),
                    ])
                    ...
                    with _viz_tabs[0]:
                        if len(_sig_ia) == 0:
                            st.info("有意な遺伝子がありません。" if _is_jp else "No significant genes found.")
                        else:
                            _top_n_ia = st.slider(
                                "表示遺伝子数" if _is_jp else "Top N genes",
                    ...
                    with _viz_tabs[1]:
                        if len(_sig_ia) == 0:
                            ...
                            _line_genes = st.multiselect(
                                "遺伝子を選択（最大8個）" if _is_jp else "Select genes (max 8)",
                            ...
                            if _line_genes and _ia_var_viz and _ia_var_viz in _meta_viz.columns:
                                ...
                                st.info(
                                    "遺伝子と交互作用変数を選択してください。"
                                    if _is_jp else
                                    "Select genes and ensure interaction variable is set."
                                )
                    ...
                    with _viz_tabs[2]:
                        if _ia_var_viz and _ia_var_viz in _meta_viz.columns:
                            ...
                            if len(_var_vals_unique) >= 2:
                                ...
                                st.caption(
                                    "💡 " + (
                                        "なげなわ/ボックスでドットを囲むと選択遺伝子がLine Plotに反映されます。右上のツールバーでlassoまたはboxを選択してください。"
                                        if _is_jp else
                                        "Lasso or box-select dots to send selected genes to the Line Plot. Choose lasso or box tool from the top-right toolbar."
                                    )
                                )
                                ...
                                for (_va, _vb) in _lfc_pairs:
                                    ...
                                    if _HAS_PLOTLY_EVENTS:
                                        ...
                                        if st.session_state.get("ia_lasso_genes"):
                                            st.info(
                                                "💡 なげなわ選択を有効にするには `pip install streamlit-plotly-events` を実行してください。"
                                                if _is_jp else
                                                "💡 To enable lasso selection, run `pip install streamlit-plotly-events`."
                                            )
                                ...
                                if st.session_state.get("ia_lasso_genes"):
                                    if st.button(
                                        "🗑️ 選択をクリア" if _is_jp else "🗑️ Clear selection",
                                        key="ia_lasso_clear"
                                    ):
                                        st.session_state["ia_lasso_genes"] = []
                                        st.rerun()
                            ...
                            st.info(
                                "交互作用変数が設定されていません。Uploadタブで追加変数を設定してください。"
                                if _is_jp else
                                "No interaction variable set. Please configure additional variables in the Upload tab."
                            )
    ...
    if st.session_state["deg_results"] is None:
        ...
        def render_plot_settings(key_suffix):
            with st.expander("⚙️ Plot Settings", expanded=False):
                ...
                with f_c1:
                    sel_font = st.selectbox("Font", _dynamic_fonts, index=_dynamic_fonts.index(st.session_state.get("selected_font", "sans-serif")), key=f"viz_font_{key_suffix}")
                    img_format = st.selectbox("Export format", ["png", "pdf", "svg"], index=["png", "pdf", "svg"].index(st.session_state.get("img_format", "png")), key=f"viz_fmt_{key_suffix}")
                    img_dpi = st.selectbox("DPI", [300, 600], index=[300, 600].index(st.session_state.get("img_dpi", 300)), key=f"viz_dpi_{key_suffix}")
                with f_c2:
                ...
                fig_width = st.slider("Width (px)", 400, 1600, st.session_state.get("fig_width", 800), 50, key=f"viz_w_{key_suffix}")
                fig_height = st.slider("Height (px)", 300, 1200, st.session_state.get("fig_height", 500), 50, key=f"viz_h_{key_suffix}")
                fig_font_sz = st.slider("Font size (pt)", 8, 28, st.session_state.get("fig_font_sz", 12), 1, key=f"viz_sz_{key_suffix}")
                ...
                if st.session_state.get("conditions"):
                    with st.expander("Group Colors", expanded=False):
                ...
                with st.expander("Analysis Plot Colors", expanded=False):
                    _ecmap_opts = ["Viridis_r", "Plasma_r", "Magma_r", "Inferno_r", "Cividis_r"]
                    _hcmap_opts = ["RdBu_r", "Spectral_r", "Coolwarm", "RdYlBu_r"]
                    enr_cmap = st.selectbox("Enrichment Scale", _ecmap_opts, index=_ecmap_opts.index(st.session_state.get("enr_cmap", "Viridis_r")), key=f"viz_ecmap_{key_suffix}")
                    hm_cmap = st.selectbox("Heatmap Scale", _hcmap_opts, index=_hcmap_opts.index(st.session_state.get("hm_cmap", "RdBu_r")), key=f"viz_hcmap_{key_suffix}")
...
with tab_network:
    ...
    if st.session_state["deg_results"] is None:
        ...
        with st.expander("🎨 Figure Settings", expanded=False):
            ...
            f_c1, f_c2 = st.columns(2)
            with f_c1:
                sel_font = st.selectbox("Font", _dynamic_fonts, index=_dynamic_fonts.index(st.session_state.get("selected_font", "sans-serif")), key="net_font")
                img_format = st.selectbox("Export format", ["png", "pdf", "svg"], index=["png", "pdf", "svg"].index(st.session_state.get("img_format", "png")), key="net_fmt")
                img_dpi = st.selectbox("DPI", [300, 600], index=[300, 600].index(st.session_state.get("img_dpi", 300)), key="net_dpi")
            with f_c2:
            ...
            fig_width = st.slider("Width (px)", 400, 1600, st.session_state.get("fig_width", 800), 50, key="net_w")
            fig_height = st.slider("Height (px)", 300, 1200, st.session_state.get("fig_height", 500), 50, key="net_h")
            fig_font_sz = st.slider("Font size (pt)", 8, 28, st.session_state.get("fig_font_sz", 12), 1, key="net_sz")
            ...
            with st.expander("Analysis Plot Colors", expanded=False):
                _ecmap_opts = ["Viridis_r", "Plasma_r", "Magma_r", "Inferno_r", "Cividis_r"]
                _hcmap_opts = ["RdBu_r", "Spectral_r", "Coolwarm", "RdYlBu_r"]
                enr_cmap = st.selectbox("Enrichment Scale", _ecmap_opts, index=_ecmap_opts.index(st.session_state.get("enr_cmap", "Viridis_r")), key="net_ecmap")
                hm_cmap = st.selectbox("Heatmap Scale", _hcmap_opts, index=_hcmap_opts.index(st.session_state.get("hm_cmap", "RdBu_r")), key="net_hcmap")
                st.session_state["enr_cmap"] = enr_cmap
                st.session_state["hm_cmap"] = hm_cmap
...
""")
            ...
            if st.button("Calculate GSEA", type="primary", disabled=_gsea_disabled):
                try:
                    ...
                    with st.status("🎩 GSEA解析中...", expanded=True):
...
""")
            ...
            if st.session_state.get("norm_method") in ["CPM", "TPM"]:
                st.caption(
                    "ℹ️ CPM/TPM data will be log(x+1) transformed before TF activity "
                    "estimation, as decoupleR requires log-scale input."
                )
            ...
            if st.button("Estimate TF Activity", type="primary", disabled=_tf_disabled):
                try:
                    ...
                    with st.status("🎩 TF活性解析中...", expanded=True) as status:
                        # ── decoupler v2.x API ──────────────────────────────
        ...
        with nt4:
            st.subheader("Cell-type Scoring / Deconvolution")
            with st.expander("About Cell-type Scoring", expanded=False):
                if _is_jp:
                    st.markdown("""
...
""")
            ...
            if _FAILED_REFS:
                for _fname, _ferr in _FAILED_REFS:
                    st.warning(
                        f"⚠️ 外部参照ファイルの読み込みに失敗しました: `{_fname}.csv` — {_ferr}"
                        if _is_jp else
                        f"⚠️ Failed to load external reference: `{_fname}.csv` — {_ferr}"
                    )
            ...
            if _n_total_samples < 6:
                st.warning("⚠️ " + (f"サンプル数が{_n_total_samples}件です。デコンボリューションは6サンプル以上を推奨します（サンプル数が少ないと細胞型推定の精度が低下します）。" if _is_jp else f"Only {_n_total_samples} samples. Deconvolution recommends 6+ samples for reliable cell type estimation."))
            if st.button("Deconvolve", type="primary", disabled=_decon_disabled):
                try:
                    ...
                    if ref_choice == "mMCP-counter (Mouse recommended 🐭)":
                        ...
                        st.info("🐭 mMCP-counter はマウスデータ専用です。遺伝子名が Cd8a 形式（先頭大文字）である必要があります。出力は割合ではなく存在量スコアです。引用: Petitprez et al., Genome Medicine (2020)" if _is_jp else
                                "🐭 mMCP-counter is designed for mouse data. Gene names must be in 'Cd8a' format. Output is abundance score, not fraction. Citation: Petitprez et al., Genome Medicine (2020)")
                        ...
                        if _ext_key in _EXTERNAL_REFS:
                            ...
                            is_builtin = False
                            st.info(f"📁 External reference loaded: {_ext_key} "
                                    f"({ref_df_used.shape[0]} genes × {ref_df_used.shape[1]} cell types)")
                        else:
                            st.error(f"Reference file not found: {_ext_key}")
                            st.stop()
                    ...
                    if len(common_genes) < 5:
                        ...
                        with st.status(f"🎩 デコンボリューション処理中 ({len(counts_decon.columns)} samples)..."):
            ...
            if st.session_state["ciber_results"] is not None:
                ...
                st.caption(f"{_res_label} for {ciber_df.shape[0]} samples, {ciber_df.shape[1]} cell types")
        ...
        with nt_corr:
            if st.session_state.get("counts_df") is None:
                if st.session_state.get("lang_display", "日本語") == "日本語":
                    ...
                    st.info("💡 **No data available**\n\nPlease upload data in the **Upload** tab first.")
            else:
                st.subheader("🔗 Gene Correlation Analysis" if _is_jp else "🔗 Gene Correlation Analysis")
...
""")
                ...
                _n_total_samples = st.session_state["counts_df"].shape[1] if st.session_state.get("counts_df") is not None else 0
                if _n_total_samples < 6:
                    st.warning("⚠️ " + (f"サンプル数が{_n_total_samples}件です。相関解析は6サンプル以上を推奨します（6未満では相関係数・p値の信頼性が低下します。安定した推定には10サンプル以上が望ましいです）。" if _is_jp else f"Only {_n_total_samples} samples. Correlation analysis recommends 6+ samples (10+ samples are often preferable for more stable estimates). Results may be unreliable with fewer samples."))
...
with tab_meta:
    ...
    if len(_batch) < 2:
        ...
        st.warning(
            "⚠️ **統計的注意**: Fisher法によるp値統合は、各コントラストのp値が**統計的に独立**であることを前提とします。"
            "同一Study内で共通のコントロール群を持つコントラスト（例：Drug A vs Control と Drug B vs Control）を統合すると、"
            "独立性の前提が破綻し偽陽性が増加します。**独立した別々のStudy間でのみ使用してください。**"
            if _is_jp else
            "⚠️ **Statistical note**: Fisher's method for combining p-values requires that each contrast is **statistically independent**. "
            "Combining contrasts that share a common control group within the same study (e.g., Drug A vs Control and Drug B vs Control) "
            "violates this independence assumption and inflates false positives. "
            "**Use only across independent studies.**"
        )
        _mc1, _mc2, _mc3 = st.columns(3)
        with _mc1:
        ...
        with _mc3:
            _min_studies_ui = st.slider(
                "最低検出スタディ数" if _is_jp else "Min. studies detected",
                min_value=2,
            ...
            _meta_padj_t = st.number_input(
                "統合p値の閾値" if _is_jp else "Meta padj threshold",
                0.0, 1.0, 0.05, 0.005, key="meta_padj_t"
            )
            _meta_run = st.button("プロット" if _is_jp else "Plot", key="meta_plot_btn", type="primary", use_container_width=True)
